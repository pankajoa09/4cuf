...
const siteUrl = 'https://www.4cuf.com';
...
<body>
<main class="wrap">
  <div class="top"><a href="/"><img class="logo" src="/logo.svg" alt="4CUF"></a><a class="back" href="/">← All Thailand car events</a></div>
  <article class="event">
...
function sitemap(data) {
  const pages = [
    ...
    ...data.events
      ...
      .map(event => ({ url: `${siteUrl}/events/${encodeURIComponent(event.id)}/`, lastmod: event.last_verified }))
  ];
...
if (checkOnly) {
  ...
  if (stale.length) {
    ...
    process.exitCode = 1;
  } else console.log(`Validated ${data.events.length} events; homepage, event pages, sitemap, and robots.txt are in sync.`);
} else {
  ...
  console.log(`Validated and synced ${data.events.length} events plus event pages, sitemap, and robots.txt.`);
}
